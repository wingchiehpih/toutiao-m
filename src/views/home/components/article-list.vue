...
export default {
  ...
  data() {
    return {
      ...
      // 控制加载中 loading 状态
      loading: false,
      // 控制数据加载结束的状态，当所有数据加载完成后finish会变成true此时显示内容为加载完成
      // 当数据加载完成后设置为true
      finished: false,
      // 请求获取下一页数据的时间戳
      timestamp: null,
    ...
    async onLoad() {
      try {
        ...
        const { data } = await getArticles({
          ...
          // 判断如果没有this.timestamp没有数据则使用Date.now()中的数据
        ...
        // 如果本次请求到数据，证明可能后面还有
        if (results.length) {
          // 更新获取下一页数据的时间戳，请求结束后将返回值中的timestamp赋值，用于下一次获取数据
